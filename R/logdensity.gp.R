...
function(object,...){
  ...
  if(object$d==2){
    ...
    coeff.vec=c(a1r,a2r,a3r,a4r,b1r,b2r,b3r,b4r,b1i,b2i,b3i,b4i,c1r,c1i,c2r,c2i)
    # create vector of component variances
    variance.vec=c(object$variances[1,1],object$variances[(m1/2+1),1],object$variances[1,(m2/2+1)],object$variances[(m1/2+1),(m2/2+1)],object$variances[2:(m1/2),1],object$variances[1,2:(m2/2)],object$variances[(m1/2+1),2:(m2/2)],object$variances[2:(m1/2),(m2/2+1)],object$variances[2:(m1/2),1],object$variances[1,2:(m2/2)],object$variances[(m1/2+1),2:(m2/2)],object$variances[2:(m1/2),(m2/2+1)],object$variances[2:(m1/2),2:(m2/2)],object$variances[2:(m1/2),2:(m2/2)],object$variances[(m1/2+2):m1,2:(m2/2)],object$variances[(m1/2+2):m1,2:(m2/2)])
  } else{
    coeff.vec=c(a1r,a2r,b1r,b1i)
    variance.vec=c(object$variances[1,1],object$variances[(m1/2+1),1],object$variances[2:(m1/2),1],object$variances[2:(m1/2),1])
  }
  # normal prior log density
  return(sum(dnorm(coeff.vec,0,sqrt(variance.vec),log=TRUE)))
}
